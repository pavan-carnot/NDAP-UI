  ...
  return (
    <div className="flex-1 flex max-w-screen-xl mx-auto w-full px-4 py-4 gap-4 overflow-hidden min-h-0">
      ...
      <aside className="hidden lg:flex flex-col w-64 xl:w-72 flex-shrink-0 gap-3 overflow-y-auto">
        ...
        {/* Recent queries */}
        <div className="bg-white border border-ndap-border rounded-xl p-3 shadow-sm flex-1 overflow-auto">
          ...
          {recentQueries.length > 0 ? (
            <div className="space-y-1">
              {recentQueries.map((rq) => (
                <button
                  ...
                  className="w-full text-left text-xs text-gray-700 hover:text-ndap-blue py-1.5 px-2 rounded-lg hover:bg-ndap-sky transition-colors line-clamp-2 leading-snug"
                >
                  {rq.text}
                </button>
              ))}
            ...
            <p className="text-xs text-gray-400 italic">No recent queries yet.</p>
          )}
        </div>
